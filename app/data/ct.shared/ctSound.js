...
var import_sound = require("node_modules/@pixi/sound");
var _a;
...
var fxNames = Object.keys(import_sound.filters).filter((name) => name !== "Filter" && name !== "StreamFilter");
var fxNamesToClasses = {};
for (const fxName of fxNames) {
  fxNamesToClasses[fxName] = import_sound.filters[fxName];
}
var pixiSoundPrefix = "pixiSound-";
var randomRange = (min, max) => Math.random() * (max - min) + min;
var withSound = (name, fn) => {
  const pixiFind = import_sound.sound.find(name);
  if (pixiFind) {
    return fn(pixiFind);
  }
...
var playVariant = (sound, variant, options) => {
  var _a2, _b, _c, _d, _e;
  const pixiSoundInst = import_sound.sound.find(`${pixiSoundPrefix}${variant.uid}`).play(options);
