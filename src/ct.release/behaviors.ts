...
const behaviorsLib = {
    ...
    add(target: Room | Copy, behavior: string): void {
        ...
        const domain = target instanceof Copy ? 'templates' : 'rooms';
     ...
     */
    has(target: Room | Copy, behavior: string): boolean {
        return target.behaviors.includes(behavior);
    }
};

export const runBehaviors = (target: Room | Copy, domain: 'rooms' | 'templates', kind: keyof ExportedBehaviorDynamic): void => {
