scripts-panel
    ...
    ul.menu.flexfix-body
        li(each="{script in currentProject.scripts}" name="{script.name}" onclick="{selectScript}" ondragstart="{dragStart}" ondragend="{dragEnd}" ondragover="{dragOver}" ondrop="{drop}" draggable="true")
            code {script.name}
